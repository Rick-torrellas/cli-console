 ...
 */
 ...
 if (verbose) {  //TODO: se puede eliminar los espacios en blanco, que sean mayores de 1 espacio, con regegular exprecion y la funcion replace.
  let {
    colorName = "#FBFEFC",
    colorMessage = "#FBFEFC",
...
function command_done__Pick_Color({
  ...
  bgCheck_mark !== ""
    ? (_check_mark = chalk.bgHex(bgCheck_mark).hex(colorCheck_mark)(
        check_mark
      ))
 ...
 * @returns {void}
 */
 function process_start({ name = "",description="",verbose = false } = {},opts={}) {
  if (verbose) {
    let {colorName = "",colorDescription = "",colorTitle = "#FCFAF7",bgName = "",bgDescription="",bgTitle= "#85BBD6",titleTxt = "Process Name:"} = opts;
    let {_name,_description,_title} = process_start__Pick_Color({colorName,colorDescription,colorTitle,bgName,bgDescription,bgTitle,name,description,titleTxt});
// PROCESS
...
function process_start__Pick_Color({colorName,colorDescription,colorTitle,bgName,bgDescription,bgTitle,name,description,titleTxt}) {
  ...
  bgName !== ""
    ...
    : (_name = chalk.hex(colorName)(name));
    bgDescription !== ""
    ? (_description = chalk.bgHex(bgDescription).hex(colorDescription)(description))
    : (_description = chalk.hex(colorDescription)(description));
    return {
      _name,
      _description,
 ...
 */
 function info({name = "", message = ""}={},opts={}) {
  let {colorName="#FEFBFB",colorMessage="#FEFBFB",colorInfo="#2E578A",bgName="",bgMessage="",bgInfo="",infoTxt="Info"} = opts;
  let {_name,_message,_info} = info_Pick_Color({name,message,infoTxt,colorName,colorMessage,colorInfo,bgName,bgMessage,bgInfo});
  if (name === "" && message === "") {
  return;
  }
  log(`${_info}: ${_name}`);
...
function info_Pick_Color({colorName,colorMessage,colorInfo,bgName,bgMessage,bgInfo,name,message,infoTxt}) {
 ...
 */
 function error({error=""}={},opts={}) {
  ...
  let {colorError="#FBF4F4",colorTitle="FBF4F4",bgError="",bgTitle="#B43C3C",titleText="ERROR"} = opts;
  let {_title,_error} = error_Pick_Color({colorError,colorTitle,bgError,bgTitle,titleText,error});
  if (error === "") {
    return;
  }
  if (
  ...
  function error_Pick_Color({colorError,colorTitle,bgError,bgTitle,titleText,error}) {
    let _title,_error;
 ...
 */
 function warning(
  { name="",message="" , verbose = false} = {},opts={}
) {
  if (verbose) {
    let {colorName="#FBF4F4",colorMessage="#FBF4F4",colorTitle="#FBF4F4",bgName="",bgMessage="",bgTitle="#B9C34B",titleTxt="WARNING"} = opts
    let {_name,_message,_title} = warning_Pick_Color({colorName,colorMessage,colorTitle,bgName,bgMessage,bgTitle,titleTxt,name,message});
    if (name === "" && message === "") {
      return;
    }
    log(`${_title}: ${_name}`);
...
function warning_Pick_Color({colorName,colorMessage,colorTitle,bgName,bgMessage,bgTitle,titleTxt,name,message}) {
